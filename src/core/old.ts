...
export class AuthRole {
  ...
  public isUser = async (req: ICustomRequest, _: Response, next: NextFunction) => {
  ...
  public refetchToken = async (req: ICustomRequest, res: Response): Promise<boolean> => {
    try {
      const refreshToken = req.cookies.refreshToken

      const decodedRefreshToken: any = await this.jwtService.verifyRefreshToken(refreshToken)
      const { _id } = decodedRefreshToken
      console.log('🚀 hello cac ban tre  ~ file: index.ts:29 ~ readonlyrefetchToken= ~ _id~', _id)
      ...
      const refreshTokenExpiresString = await getJwtSetting(this.context)('jwt_refresh_token_expires')
